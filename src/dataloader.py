...
class SubjectWiseDataLoader:
    ...
    def split_subjects(self) -> dict[str, pd.DataFrame]:
        ...
        for split_name, split_df in self.splits.items():
            ...
            log.info(
                f"{split_name}: {n_rows} rows ({pct_rows:.1f}%), "
                f"{n_subj} subjects ({pct_subj:.1f}%), labels: {label_dist}"
            )
    ...
    def get_loso_fold(
        ...
        if fold_idx < 0 or fold_idx >= len(folds):
            raise ValueError(f"Invalid fold_idx {fold_idx}. Must be in range [0, {len(folds) - 1}]")
...
def load_loso_folds(
    ...
    Example:
        ...
        ...     y_train = df_train['pseudo_label_smoothed'].values
        ...     X_test = df_test[features].values
        ...     y_test = df_test['pseudo_label_smoothed'].values
    """
    loader = SubjectWiseDataLoader(
        csv_path=csv_path,
        feature_cols=feature_cols,
...
if __name__ == "__main__":
    ...
    for fold_idx in range(min(3, len(folds))):
        fold = folds[fold_idx]
        X_train, y_train = (
            fold["train"][loader.feature_cols].values,
            fold["train"]["pseudo_label_smoothed"].values,
        )
        X_test, y_test = (
            fold["test"][loader.feature_cols].values,
            fold["test"]["pseudo_label_smoothed"].values,
        )
