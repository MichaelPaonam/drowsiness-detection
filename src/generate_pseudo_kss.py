...
def run_kmeans(
    ...
    log.info("KMeans silhouette=%.4f  cluster sizes=%s",
             sil, dict(zip(*np.unique(ordered, return_counts=True))))
...
def run_gmm(
    ...
    ordered    = perm[raw_labels]
    sil        = float(silhouette_score(X_scaled, raw_labels))
    log.info("GMM silhouette=%.4f  cluster sizes=%s",
             sil, dict(zip(*np.unique(ordered, return_counts=True))))
...
def _cluster_colors(ordered_labels: np.ndarray) -> list[str]:
    names   = [CLUSTER_NAMES[l] for l in ordered_labels]
    return [CLUSTER_COLORS[n] for n in names]


def fig_cluster_scatter(
    df: pd.DataFrame, X_scaled: np.ndarray,
    labels_km: np.ndarray | None, labels_gmm: np.ndarray | None,
) -> None:
    ...
    for ax, (method, labels) in zip(axes, [
        ("KMeans", labels_km), ("GMM", labels_gmm)
    ]):
        if labels is None:
            continue
...
def fig_cluster_centroids(
    ...
    panels = [(c, m) for c, m in [
        (centroids_km, "KMeans"), (centroids_gmm, "GMM")
    ] if c is not None]
    if not panels:
        return
    ...
    for ax, (centroids, method) in zip(axes, panels):
        for ci in range(n_clusters):
            ...
            ax.bar(offsets, centroids[ci], width * 0.9, color=bar_colors[ci],
...
def fig_silhouette(
    ...
    panels = [(l, m) for l, m in [
        (labels_km, "KMeans"), (labels_gmm, "GMM")
    ] if l is not None]
    ...
    for ax, (labels, method) in zip(axes, panels):
        ...
        for ci in range(n_clusters):
            ...
            ax.fill_betweenx(np.arange(y_lower, y_upper), 0, ci_sil,
                             color=bar_colors[ci], alpha=0.7)
            ax.text(-0.05, (y_lower + y_upper) / 2,
                    CLUSTER_NAMES[ci][:3], fontsize=8, va="center")
            y_lower = y_upper + 10

        avg = float(silhouette_score(X_scaled, labels))
        ax.axvline(avg, color="red", linestyle="--", linewidth=1.2)
        ax.text(avg + 0.01, y_lower * 0.95, f"avg={avg:.3f}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate pseudo-KSS labels via HRV clustering"
    )
